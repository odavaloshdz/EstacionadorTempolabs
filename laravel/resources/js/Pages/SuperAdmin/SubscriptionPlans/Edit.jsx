...
export default function Edit({ auth, subscriptionPlan }) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        billing_cycle: subscriptionPlan.billing_cycle || 'monthly',
        duration_in_days: subscriptionPlan.duration_in_days || 30,
        max_users: subscriptionPlan.max_users || 5,
        max_parking_spaces: subscriptionPlan.max_parking_spaces || 10,
    ...
    useEffect(() => {
        // Parse features from string to array when component mounts
        if (subscriptionPlan.features) {
            setData('features', subscriptionPlan.features.split(',').map(feature => feature.trim()));
        }
    }, []);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('superadmin.subscription-plans.update', subscriptionPlan.id));
    };
        ...
        <SuperAdminLayout
            ...
            header={
                <div className="flex items-center">
                    ...
                    <h2 className="font-semibold text-xl text-gray-800 leading-tight">Editar Plan de Suscripción: {subscriptionPlan.name}</h2>
                </div>
            }
        >
            <Head title={`Editar Plan: ${subscriptionPlan.name}`} />
